...
***Note: Remeber to change the --iteration or -i value to smaller number or else it will take hours to optimize it if you set it for blind docking <br>
...
pandadock -p protein.pdb -l ligand.mol -a genetic -i 100 -o results
...
```bash
pandadock -p protein.pdb -l ligand.sdf -s 110.133 135.572 141.959 -r 3.0 \
  --flex-residues A_42 A_85 A_132 --max-flex-bonds 2 -i 100
```
