...
run_docking "enhanced_gpu" -p $PROTEIN -l $LIGAND -s $SITE_X $SITE_Y $SITE_Z -i 1 -a genetic --use-gpu --enhanced-scoring --gpu-id 0 --detailed-energy

# 4. Enhanced Scoring (CPU)
run_docking "enhanced_cpu" -p $PROTEIN -l $LIGAND -s $SITE_X $SITE_Y $SITE_Z -i 1 -a genetic --enhanced-scoring --detailed-energy

# 5. Fast Mode Docking
run_docking "fast_mode" -p $PROTEIN -l $LIGAND -s $SITE_X $SITE_Y $SITE_Z -i 1 -a random --fast-mode
...
run_docking "full_analysis" -p $PROTEIN -l $LIGAND -s $SITE_X $SITE_Y $SITE_Z -i 1 -a genetic --use-gpu --physics-based --gpu-id 0 \
...
run_docking "Pandadock_algorithm" $PROTEIN -l $LIGAND -s $SITE_X $SITE_Y $SITE_Z -a pandadock --use-gpu --physics-based \
  --num-conformers 1 --num-orientations 1 --md-steps 100 --minimize-steps 200 --detailed-energy \
  --report-format all 