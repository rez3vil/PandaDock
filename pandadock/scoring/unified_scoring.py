...
try:
    ...
    # Fallback implementation of cdist if scipy not available
    def cdist(XA, XB, metric='euclidean'):
        """Simple fallback for cdist when scipy is not available."""
        if metric != 'euclidean':
            raise NotImplementedError("Only euclidean metric supported in fallback")
        XA = np.array(XA)
        XB = np.array(XB)
        distances = np.sqrt(((XA[:, np.newaxis, :] - XB[np.newaxis, :, :]) ** 2).sum(axis=2))
        return distances
import time
import warnings
...
try:
    ...
    # Fallback to scoring module
    try:
        from .scoring.physics_based_scoring import PhysicsBasedScoringFunction
        PhysicsBasedScoring = PhysicsBasedScoringFunction
    except ImportError:
        # Create dummy classes if physics not available
        class PhysicsBasedScoring:
            pass
        class PhysicsBasedScoringFunction:
            pass
